...
echo -e "Running renamig using renameToHLscaffolds.py....."
renameToHLscaffolds.py -c 1 -a $ANNODICT -d <(awk '{print $2","$1}' $NAMESFILE) | grep -v  ^reg_ | grep -v ^rna- | awk '{print $2","$1}' | sort -u > $NAMESDICT
